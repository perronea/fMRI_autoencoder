...
def train_classifier(train_pconns, train_labels, test_pconns, test_labels):
    
    classifier = Sequential()
    classifier.add(Dense(432, input_dim=61776, activation='relu'))
    ...
    training = classifier.fit(train_pconns, train_labels, validation_split=0.2, epochs=20, batch_size=10)

    plot_training(training)

    _, accuracy = classifier.evaluate(test_pconns, test_labels)
...
def main():
    ...
    train_classifier(train_pconns, train_labels, test_pconns, test_labels)
